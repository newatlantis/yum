...
property :install_weak_deps, [true, false], default: true, description: "(DNF only) When this option is set to true and a new package is about to be installed, all packages linked by a weak dependency relation (i.e., Recommends or Supplements flags) with this packgage will be pulled into the transaction. Default is DNF's default of true."
...
action :create do
  template new_resource.path do
    # Use DNF if on CentOS/RHEL 8 or later
    helper(:use_dnf) do
      # NOTE: The platform? method appears to be unavailable in this context
      # rubocop:disable Chef/Style/UsePlatformHelpers
      %w(centos redhat).include?(node['platform']) &&
        node['platform_version'].split(/\./)[0].to_i >= 8
      # rubocop:enable Chef/Style/UsePlatformHelpers
    end
    source 'main.erb'
    cookbook 'yum'
    mode '0644'
